...
2. **Поиск по извлечённым текстам**:
   ...
   - **Описания кадров**: Кадры описываются моделью BLIP, затем spaCy извлекает сущности и отношения.
...
- **Разработка универсальной системы, объединение ASR, OCR и описаний кадров, поиск по Qdrant и объединение результатов (RRF, Max-per-Modality), API/веб-интерфейс, очистка от прототипного кода, оценка качества**: Максим Ляра.
