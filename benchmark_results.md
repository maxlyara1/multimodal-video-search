...
- **Git Commit**: `54fe6ad63db4b079868bc9a9889a8129c6123653`
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| Только ASR | Сумма | Нет | 66.00% | 80.00% | 0.302s |
| Мультимодальный | Простая сумма | Задан | 70.00% | 94.00% | 0.442s |
| Мультимодальный | RRF | Задан | 88.00% | 94.00% | 0.443s |
| **Мультимодальный** | **Максимум каждой модальности** | **Задан** | **90.00%** | **96.00%** | 0.454s |
| Мультимодальный | Максимум каждой модальности | Нет | 76.00% | 88.00% | 0.770s |
...
2. **Влияние выбора модальностей (Абляция)**: На проверочном наборе выбор активных модальностей совместно с методом Max-per-Modality повысил Hit@1 с 76.00% (без выбора модальностей) до 90.00%. Это показывает пользу ограничения активных каналов для снижения влияния шума нерелевантных модальностей. Автоматический выбор модальностей моделью Gemini в этом эксперименте не оценивался.
...
4. **Время поиска (Локальное время)**: Среднее время локального поиска (включая векторизацию и объединение результатов) составляет для базового варианта 0.302s, а для мультимодальных режимов колеблется в диапазоне от 0.442s до 0.454s.
