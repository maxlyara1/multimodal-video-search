...
- **Git Commit**: `78fa0db4b341a498debd3a24b132a0bc1834acef`
...
| :--- | :--- | :---: | :---: | :---: | :---: |
| Только ASR | Сумма | Нет | 66.00% | 80.00% | 0.304s |
| Мультимодальный | Простая сумма | Задан | 70.00% | 94.00% | 0.435s |
| Мультимодальный | RRF | Задан | 88.00% | 94.00% | 0.439s |
| **Мультимодальный** | **Максимум каждой модальности** | **Задан** | **90.00%** | **96.00%** | 0.454s |
| Мультимодальный | Максимум каждой модальности | Нет | 76.00% | 88.00% | 0.767s |
...
| `VISUAL` | 20 | 50.00% | 75.00% | 95.00% | **95.00%** | 60.00% |
...
1. **Сравнение стратегий объединения**: На проверочном наборе с зафиксированной декомпозицией запросов объединение по максимуму (Max-per-Modality) в сочетании с выбором модальностей показало наилучший точечный результат: Hit@1 вырос до 90.00% (+24 процентных пунктов относительно базового варианта 66.00%), а Hit@3 достиг 96.00% (+16 процентных пунктов относительно базового варианта 80.00%). Разница с RRF (88.00% по Hit@1, 94.00% по Hit@3) составляет всего 1 запрос из 50, поэтому превосходство не является убедительным. Разница между двумя лучшими методами составляет один запрос из 50.
2. **Влияние выбора источников информации (Сравнение без выбора источников)**: На проверочном наборе выбор активных источников информации совместно с методом Max-per-Modality повысил Hit@1 с 76.00% (без выбора источников информации) до 90.00%. Это показывает пользу ограничения активных каналов для снижения влияния шума нерелевантных источников. Автоматический выбор источников информации моделью Gemini в этом эксперименте не оценивался.
...
4. **Время поиска (Локальное время)**: Среднее время локального поиска (включая векторизацию и объединение результатов) составляет для базового варианта 0.304s, а для мультимодальных режимов колеблется в диапазоне от 0.435s до 0.454s.
